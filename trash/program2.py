...
class ContentSearchWorker(QThread):
    ...
    def __init__(self, files: list, search_text: str):
        super().__init__()
        self.files       = files
...
class MatchHighlighter(QSyntaxHighlighter):
    def __init__(self, parent, keyword: str):
...
class FiltersTab(QWidget):
    def __init__(self):
        ...
        self.ext_list.setEnabled(False)
...
class SearchTab(QWidget):
    ...
    def _phase2(self, files: list, search_text: str):
        ...
        if self._highlighter:
            self._highlighter.update_keyword(search_text)
        else:
            self._highlighter = MatchHighlighter(self.preview.document(), search_text)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        #sub.setStyleSheet(f"color: {TEXT_SEC}; font-size: 12px; margin-top: 4px;")
        vhdr = QVBoxLayout()
        vhdr.setSpacing(0)
